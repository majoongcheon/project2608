...
| 파일 | 책임 |
|---|---|
| `ml/SJH_cluster_raw/__init__.py` | 빈 파일 (패키지 선언) |
| `ml/SJH_cluster_raw/load.py` | 원본 CSV 읽기, DB 에서 `split`·`cv_fold` 가져와 값 튜플로 조인 |
| `ml/SJH_cluster_raw/clean.py` | 버릴 컬럼 판정 (전부결측·상수·식별자·주관식·소수 블록) |
| `ml/SJH_cluster_raw/gates.py` | 결측 패턴이 같은 컬럼을 묶어 게이트 이진 변수 생성 |
| `ml/SJH_cluster_raw/scales.py` | 컬럼별 척도 판정 (binary/nominal/ordinal/continuous) |
| `ml/SJH_cluster_raw/gower.py` | Gower 거리 행렬. pairwise 결측 제외 + 블록 가중치 |
| `ml/SJH_cluster_raw/cluster.py` | PAM(k-medoids) 직접 구현 + 계층군집(scipy) |
| `ml/SJH_cluster_raw/score.py` | 실루엣·ARI·NMI·교차표·고부담 리프트·게이트 오염도 |
| `ml/SJH-cluster-raw.py` | 실행 엔트리. 위 모듈을 순서대로 호출하고 JSON 을 쓴다 |
| `ml/tests/test_SJH_cluster_raw.py` | 위 모듈들의 단위 테스트 |
...
**Files:**
- Create: `ml/SJH_cluster_raw/__init__.py`
- Create: `ml/SJH_cluster_raw/load.py`
- Test: `ml/tests/test_SJH_cluster_raw.py`

- [ ] **Step 1: 패키지 선언 파일 생성**

```bash
mkdir -p ml/SJH_cluster_raw
printf '"""원본 507컬럼 군집화 실험 (신지현, 2026-09-02)."""\n' > ml/SJH_cluster_raw/__init__.py
```

- [ ] **Step 2: 실패하는 테스트를 쓴다**

`ml/tests/test_SJH_cluster_raw.py` 를 만들고 아래를 넣는다.
...
Run: `cd ~/project2608-sjh && PYTHONPATH=ml/.pylibs-sjh:ml/.pylibs:ml python3 -m pytest ml/tests/test_SJH_cluster_raw.py -v`
Expected: FAIL — `ModuleNotFoundError: No module named 'SJH_cluster_raw.load'`
...
```bash
git add ml/SJH_cluster_raw/__init__.py ml/SJH_cluster_raw/load.py ml/tests/test_SJH_cluster_raw.py
...
**Files:**
- Create: `ml/SJH_cluster_raw/clean.py`
- Modify: `ml/tests/test_SJH_cluster_raw.py`

- [ ] **Step 1: 실패하는 테스트를 쓴다**

`ml/tests/test_SJH_cluster_raw.py` 끝에 붙인다.
...
Run: `cd ~/project2608-sjh && PYTHONPATH=ml/.pylibs-sjh:ml/.pylibs:ml python3 -m pytest ml/tests/test_SJH_cluster_raw.py -v`
Expected: FAIL — `No module named 'SJH_cluster_raw.clean'`
...
```bash
git add ml/SJH_cluster_raw/clean.py ml/tests/test_SJH_cluster_raw.py
...
**Files:**
- Create: `ml/SJH_cluster_raw/gates.py`
- Modify: `ml/tests/test_SJH_cluster_raw.py`
...
Run: `cd ~/project2608-sjh && PYTHONPATH=ml/.pylibs-sjh:ml/.pylibs:ml python3 -m pytest ml/tests/test_SJH_cluster_raw.py -v`
Expected: FAIL — `No module named 'SJH_cluster_raw.gates'`
...
```bash
git add ml/SJH_cluster_raw/gates.py ml/tests/test_SJH_cluster_raw.py
...
**Files:**
- Create: `ml/SJH_cluster_raw/scales.py`
- Modify: `ml/tests/test_SJH_cluster_raw.py`
...
Run: `cd ~/project2608-sjh && PYTHONPATH=ml/.pylibs-sjh:ml/.pylibs:ml python3 -m pytest ml/tests/test_SJH_cluster_raw.py -v`
Expected: FAIL — `No module named 'SJH_cluster_raw.scales'`
...
```bash
git add ml/SJH_cluster_raw/scales.py ml/tests/test_SJH_cluster_raw.py
...
**Files:**
- Create: `ml/SJH_cluster_raw/gower.py`
- Modify: `ml/tests/test_SJH_cluster_raw.py`
...
Run: `cd ~/project2608-sjh && PYTHONPATH=ml/.pylibs-sjh:ml/.pylibs:ml python3 -m pytest ml/tests/test_SJH_cluster_raw.py -v`
Expected: FAIL — `No module named 'SJH_cluster_raw.gower'`
...
```bash
git add ml/SJH_cluster_raw/gower.py ml/tests/test_SJH_cluster_raw.py
...
**Files:**
- Create: `ml/SJH_cluster_raw/cluster.py`
- Modify: `ml/tests/test_SJH_cluster_raw.py`
...
Run: `cd ~/project2608-sjh && PYTHONPATH=ml/.pylibs-sjh:ml/.pylibs:ml python3 -m pytest ml/tests/test_SJH_cluster_raw.py -v`
Expected: FAIL — `No module named 'SJH_cluster_raw.cluster'`
...
```bash
git add ml/SJH_cluster_raw/cluster.py ml/tests/test_SJH_cluster_raw.py
...
**Files:**
- Create: `ml/SJH_cluster_raw/score.py`
- Modify: `ml/tests/test_SJH_cluster_raw.py`
...
Run: `cd ~/project2608-sjh && PYTHONPATH=ml/.pylibs-sjh:ml/.pylibs:ml python3 -m pytest ml/tests/test_SJH_cluster_raw.py -v`
Expected: FAIL — `No module named 'SJH_cluster_raw.score'`
...
```bash
git add ml/SJH_cluster_raw/score.py ml/tests/test_SJH_cluster_raw.py
