...
const CONFIG = {
  ...
  'undecidable.noticeText': {
    // text 는 FR-021j-1 이 요구하는 두 가지(판정 미성립 + 안전망 목적)를 담는다. 항상 먼저 나온다.
    ...
    // FR-009a — "판정이 어렵다는 사실과 **그 이유**를 보호자가 이해할 수 있는 말로".
    //   추론 서비스가 주는 두 갈래를 그대로 문구로 옮긴다. 내부 용어(희소성·확신도)는 쓰지 않는다.
    //   아래 세 항목은 지우면 화면에서 통째로 빠지고 예전 화면으로 돌아간다.
    reasons: {
      sparse: '비슷한 상황에 계신 분들의 응답이 아직 충분히 모이지 않아, 이번 응답만으로는 부담 수준을 가늠하기 어렵습니다.',
      ambiguous: '응답해 주신 내용이 여러 부담 수준에 걸쳐 있어, 어느 한쪽으로 좁히기 어렵습니다.',
    },
    // 판정 불가를 '부담 없음'으로 읽는 것도 막는다. FR-021j-1 은 반대 방향만 막고 있다.
    meaning: '이는 부담이 없다는 뜻도, 부담이 크다는 뜻도 아닙니다. 이번 응답만으로는 판단을 내리지 않았다는 뜻입니다.',
    // 답을 바꿔 다시 시도하라는 뜻으로 읽히지 않게 쓴다.
    nextSteps: [
      '아래 안내된 기관에 연락하실 때 이 화면을 그대로 보여 주셔도 됩니다.',
      '같은 내용으로 다시 진단하셔도 결과는 같습니다. 상황이 달라졌을 때 다시 이용해 주세요.',
    ],
  },
